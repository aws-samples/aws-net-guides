        ...
        public async Task<ActionResult> Upload(HttpPostedFileBase file)
        {
            try
            {
                ...
                if (moderationResults.ImageAllowed)
                {
                    ...
                    {OrigionalFileName = file.FileName, FileType = Path.GetExtension(file.FileName), KeyName = s3KeyName, TimeStamp = DateTime.Now, ImageURL = String.Format("https://{0}/{1}", new[]{_configuration.CloudFrontDNS, s3KeyName})};
